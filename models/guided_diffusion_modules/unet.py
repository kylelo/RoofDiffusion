...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        if not (dilations is not None and len(channel_mults) == len(dilations)):
            dilations = [1] * len(channel_mults)
        ...
        ds = 1
        for level, (mult, dilation) in enumerate(zip(channel_mults, dilations)):
